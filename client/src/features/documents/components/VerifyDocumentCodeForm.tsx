...
import { verifyDocumentRequest, resendDocumentRequestCode } from '../api/publicDocumentsApi'
import { type MyDocumentRequest } from '../types/DocumentRequest'

export default function VerifyDocumentCodeForm({
  requestId,
  token,
  onVerified,
}: {
  requestId: string
  token: string
  onVerified: (documentRequest: MyDocumentRequest) => void
}) {
  ...
  async function handleSubmit(e: React.SubmitEvent<HTMLFormElement>) {
    ...
    try {
      const documentRequest = await verifyDocumentRequest(requestId, token, code)
      onVerified(documentRequest)
    } catch (err) {
  ...
  async function handleResend() {
    ...
    try {
      await resendDocumentRequestCode(requestId, token)
      setResent(true)
    } catch (err) {
  ...
  return (
    <div className='rounded-lg border border-blue-200 bg-blue-50 p-6'>
      ...
      <p className='text-sm text-blue-800 mb-4'>
        Check your email — tapping "Confirm request" there is the fastest way to finish. No luck with the link?
