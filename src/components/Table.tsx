...
// Your first milestone (30 minutes)
// Build the basic appointment dashboard with:

// Appointment type Done
// Fake API DONE
// useEffect for fetching DONE
// Loading state Done
// Error state
// Empty state
// Table rendering Done

// NEXT 30min: 1PM- 1:30PM

// Search input
  // are we searching by name? which field do search for? 
  // name and provider
// Case-insensitive search DONE
// Empty state DONE
// Decide whether search should be client-side or server-side—but first, ask me clarifying questions.
...
const PatientTable = () => {
  ...
  return (
    <div className='w-full max-w-5xl mx-auto p-6'>
      <h1 className='text-2xl font-semibold text-gray-900 mb-4'>Patients Table ({count && count > 0 && count} patients...)</h1>
      {error && (
              ...
              {appointments.length !== 0
                ? (
                  <tbody className='divide-y divide-gray-200 bg-white'>
                    {appointments.map(({ id, patientName, provider, time, status }) =>
